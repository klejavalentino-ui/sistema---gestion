...
// Función auxiliar estricta: Elimina cualquier talle del SKU Base
...
export default function Inventory() {
  ...
  // Listas estrictas e inmutables desde Ajustes/Configuración
  const availableSizes = useMemo(() => {
    return settings?.sizes?.length > 0 ? settings.sizes : ['S', 'M', 'L', 'XL'];
  }, [settings?.sizes]);

  const availableLocations = useMemo(() => {
    return settings?.locations?.length > 0 ? settings.locations : ['Local Principal'];
  }, [settings?.locations]);

  const categories = useMemo(() => {
    return settings?.categories?.length > 0 ? settings.categories : ['Remeras'];
  }, [settings?.categories]);
  ...
  // Matriz de Doble Entrada ESTRICTA: [Ubicación][Talle] = Stock
  ...
  const groupedProducts = useMemo(() => {
    ...
    products.forEach((p: any) => {
      ...
      if (!groups[groupKey]) {
        ...
        cleanBase = cleanSkuStrict(cleanBase);
      ...
      // Aseguramos sumar el stock estrictamente matematico
      const stock = Number(p.stock) || 0;
      groups[groupKey].totalStock += stock;
  ...
  const handleOpenAddModal = () => {
    ...
    // Generar matriz estricta usando SOLAMENTE configuraciones globales
  ...
  const handleOpenEditModal = (productGroup: any) => {
    ...
    // Generar matriz estricta: NO inventamos ubicaciones, usamos las de la Configuración Global
    const currentMatrix: any = {};
    availableLocations.forEach((loc: string) => {
      currentMatrix[loc] = {};
      availableSizes.forEach((size: string) => {
        // Buscar el stock de esta variante exacta
  ...
  const handleSaveProduct = (e: FormEvent) => {
    ...
    Object.keys(stockMatrix).forEach(loc => {
      Object.keys(stockMatrix[loc]).forEach(size => {
        ...
        const variantSku = existingVariant?.sku || `${finalBaseSku}-${variantCounter++}`;
        ...
        if (existingVariant) {
          // Actualizamos la que ya existía
          updateProduct(existingVariant.id, payload);
        } else if (stock > 0) {
          // 🔥 REGLA ESTRICTA: SOLO creamos nuevas variantes si el stock ingresado es mayor a 0
  ...
  // 3. IMPORTACIÓN DE EXCEL ESTRICTA
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      data.forEach((row: any) => {
        ...
        let baseSku = cleanSkuStrict(row['SKU Base'] || row['Base SKU'] || row['Código'] || row['SKU BASE']);
        if (!baseSku) {
          if (existingGroup) {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden relative animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSaveProduct} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              {/* Matriz Completa ESTRICTA DE CONFIGURACIÓN */}
